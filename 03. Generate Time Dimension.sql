...
CREATE TABLE DIM_TIME
 ...
 FROM 
   TIME_RANGE
 connBy ( LEVEL - 1 ) <= MONTHS_BETWEEN ( currDate, TIME_RANGE.START_DATE);
 ...
 SELECT
   ...
   CASE 
     WHEN TO_CHAR (currDate, 'YYYYQ') = TO_CHAR (LAST_DAY(ADD_MONTHS(TIME_RANGE.START_DATE,(LEVEL-1)*3 + 2) ), 'YYYYQ') 
       THEN LAST_DAY (currDate)
     ELSE 
 ...
 FROM 
   TIME_RANGE
 connBy ( LEVEL - 1 ) <= MONTHS_BETWEEN ( currDate, TIME_RANGE.START_DATE)/3;
 ...
 SELECT
   ...
   CASE 
     WHEN EXTRACT (YEAR FROM currDate) = EXTRACT ( YEAR FROM LAST_DAY(ADD_MONTHS(TIME_RANGE.START_DATE,(LEVEL-1)*12 + 11) )) 
      THEN LAST_DAY ( currDate )
     ELSE 
 ...
 FROM 
   TIME_RANGE
 connBy ( LEVEL - 1 ) <= MONTHS_BETWEEN ( currDate, TIME_RANGE.START_DATE)/12;
